  ...
  rlsProven ??= (async () => {
    ...
    if (!row?.active || row.superuser) {
      ...
      throw new IsolationError(
        ...
        + 'זו הגדרה של התפקיד שאיתו מתחברים, לא של הסכימה — '
        + '**הרצת המיגרציה שוב לא תעזור ולעולם לא תעזור.** '
        + 'תפקיד עם BYPASSRLS מתעלם מכל מדיניות אבטחת שורות, ותפקיד ברירת המחדל '
        + 'של Neon (neondb_owner) נוצר בדיוק כך.\n\n'
        + 'התיקון: ליצור תפקיד אפליקציה ייעודי בלי BYPASSRLS ולהצביע אליו את '
        + 'DATABASE_URL. הפקודות המדויקות ב-docs/NEON.md בריפו.',
      );
    }
  })();
  return rlsProven;
